...
#include "RGBmatrixData.h"

MeLEDMatrix::MeLEDMatrix(uint8_t port_num) : port(port_num - 1) {
    u8_SCKPin = port::get_port().s1;
    u8_DINPin = port::get_port().s2;
...
void MeLEDMatrix::showClock(uint8_t hour, uint8_t minute, bool point_flag) {
    ...
    u8_Display_Buffer[4] = pgm_read_byte(&Clock_Number_font_3x8[hour].data[0]);
    u8_Display_Buffer[5] = pgm_read_byte(&Clock_Number_font_3x8[hour].data[1]);
    u8_Display_Buffer[6] = pgm_read_byte(&Clock_Number_font_3x8[hour].data[2]);
    ...
    u8_Display_Buffer[13] = pgm_read_byte(&Clock_Number_font_3x8[minute].data[0]);
    u8_Display_Buffer[14] = pgm_read_byte(&Clock_Number_font_3x8[minute].data[1]);
    u8_Display_Buffer[15] = pgm_read_byte(&Clock_Number_font_3x8[minute].data[2]);
